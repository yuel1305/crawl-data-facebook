...
function extractNumber(input) {
  ...
  return parseInt(parts[1], 10); // Trả về phần tử thứ hai của chuỗi sau khi tách
}
...
(async () => {
  ...
  try {
    ...
    await page.type("#password", "Alchd@123");

    await page.evaluate(
      () => new Promise((resolve) => setTimeout(resolve, 20000))
    );
    console.log("Waited for 20 seconds.");
    // Select all elements with class 'ant-tabs-tab'
    const tabs = await page.$$(".ant-tabs-tab");

    // Check if there are enough elements to click the second one
    if (tabs.length > 1) {
      // Click on the second tab (index 1, since the array is zero-indexed)
      await tabs[1].click();
      console.log("click on tab");
    } else {
      console.log("There are not enough tabs");
    }
    await page.waitForSelector('div[role="tabpanel"][aria-hidden="false"]', {
      visible: true,
    });

    // Click vào phần tử này
    await page.click('div[role="tabpanel"][aria-hidden="false"]');

    // Đợi một chút để xem kết quả (tùy thuộc vào trang web của bạn)
    console.log("abc");
    ...
    const spans = await page.$$(
      ".ant-row-flex.ant-row-flex-space-between.ant-row-flex-middle span"
    );

    for (const span of spans) {
      const text = await span.evaluate((el) => el.innerText.trim());
      console.log("Span Text:", text[5]);
    }
    await page.waitForSelector(".styles__PageIndex-sc-eevgvg-3.fWbeIm", {
      visible: true,
    });
    const numPage = await page.$$(".styles__PageIndex-sc-eevgvg-3.fWbeIm");
    for (const numpage1 of numPage) {
      const text = await numpage1.evaluate((el) => el.innerText.trim());
      console.log("page", text);
    }
    const numPagesToProcess = extractNumber(
      await numPage[1].evaluate((el) => el.innerText.trim())
    ); // Lấy số trang từ numPage[1]
    ...
    for (let i = 0; i < numPagesToProcess; i++) {
      ...
      for (const row of rows) {
        ...
        if (buttons.length > 0) {
          ...
          await page.waitForSelector(".res-tb", { visible: true });
    ...
    await workbook.xlsx.writeFile("output1.xlsx");
  } catch (error) {
    console.error(error);
  }
})();
